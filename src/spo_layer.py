...
class DifferentiableMVO(nn.Module):
    def __init__(self, num_assets, max_weight_per_asset=1.0, kappa=0.0, debug=False):
        """
        """
    ...
    def forward(self, predicted_returns, covariance_matrix, kappa=None):
        ...
        for i in range(batch_size):
            ...
            if not torch.any(mu_i > 1e-6):
                ...
                try:
                    if self.debug:
                        print(f"MVO DEBUG Sample {i}: mu_i: {mu_i.detach().cpu().numpy()}")
                        print(f"MVO DEBUG Sample {i}: Sigma_i: {Sigma_i.detach().cpu().numpy()}")
            ...
            if self.max_weight_per_asset < 1.0:
                ...
                if current_sum_clipped > 1e-6:
                    ...
                    if self.debug and not torch.allclose(weights_w, weights_w_clipped):
