...
csv_files_headers = {}
csv_loggers_paths = {}
log_output_dir = 'training_logs/'

def initialize_csv_loggers():
...
def sanity_check_callback(agent, episode):
    """
    A simple callback to perform sanity checks during training.
    """
    print(f"\n--- Sanity Check at Episode {episode} ---")
    # Check for NaN in actor parameters
    for name, param in agent.policy.actor_mean_layers.named_parameters():
        if torch.isnan(param).any():
            print(f"!!! WARNING: NaN found in actor parameter: {name}")
    # Check for NaN in critic parameters
    for name, param in agent.policy.critic.named_parameters():
        if torch.isnan(param).any():
            print(f"!!! WARNING: NaN found in critic parameter: {name}")
...
def main():
    ...
    DEBUG = True  # Set to True for verbose logging and debugging
    ...
    try:
        ...
        initialize_csv_loggers() # Use train_env
    except Exception as e:
    ...
    for episode in range(1, max_episodes + 1):
        ...
        if DEBUG and episode % 10 == 0:
            sanity_check_callback(agent, episode)
